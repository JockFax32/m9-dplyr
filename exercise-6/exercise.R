...
avg.arr.delay <- group_by(flights, dest) %>% 
                 summarise(delay = mean(arr_time)) %>% 
                 mutate(faa = dest) %>% 
                 left_join(airports) %>% 
                 arrange(-delay)

# clean <- filter(avg.arr.delay, !is.na(delay)) # df with no null values for $delay
...
avg.airline.delay <- group_by(flights, carrier) %>% 
